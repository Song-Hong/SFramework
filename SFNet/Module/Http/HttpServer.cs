...
namespace SFramework.SFNet.Module.Http
{
 /// <summary>
    /// HTTP服务器
    /// 基于.Net的HttpListener实现，用于在Unity中快速搭建一个轻量级HTTP服务
    /// </summary>
    public class SfHttpServer
    {
        #region 变量
        ...
        public static SfHttpServer Start(int port, Action<HttpListenerRequest, HttpListenerResponse> onRequest = null)
        {
        ...
        /// <param name="onRequest">收到请求时的回调</param>
        /// <returns>服务器实例</returns>
        public static SfHttpServer Start(string ip, int port,
            Action<HttpListenerRequest, HttpListenerResponse> onRequest = null)
        {
            var server = new SfHttpServer();
            server.IP = ip;
            server.Port = port;
            if (onRequest != null)
            {
