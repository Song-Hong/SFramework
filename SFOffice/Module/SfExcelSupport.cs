...
/* 案例
                ...
                var myExcelData = SfExcel.ReadToExcelData(filePath);
                if (myExcelData != null)
                {
...
namespace SFramework.SFOffice.Module
{
    ...
    public static class SfExcel
    {
        ...
        => Save(filePath, new SfExcelData(), isCover, isCreate);
        ...
        /// <param name="isCreate">为空创建</param>
        /// <param name="isCover">是否覆盖</param>
        public static bool Save(string filePath, SfExcelData data, bool isCover = true, bool isCreate = true)
        {
            ...
            bool isCreate = true)
        {
            ...
            foreach (var sheetData in data)
            {
                ...
                if (!isCreate)
                {
                    worksheet = package.Workbook.Worksheets[sheetData.Key] ?? package.Workbook.Worksheets.Add(sheetData.Key);
                }
                else
                {
        ...
        public static SfExcelData ReadToExcelData(string filePath)
        {
            var excelData = new SfExcelData();
            ...
            foreach (var worksheet in package.Workbook.Worksheets)
            {
                ...
                var sheetData = new string[rowCount, colCount];
                for (int r = 0; r < rowCount; r++)
                {
                    for (int c = 0; c < colCount; c++)
                    {
                        // ExcelPackage的Cells是1-based索引
                        sheetData[r, c] = worksheet.Cells[r + 1, c + 1].Value?.ToString();
                    }
                }
                excelData.Add(new SheetData(sheetData, sheetName));
            }
        ...
        private static List<string> DataSet2StringList(DataSet result)
        {
            var data = new List<string>();
            if (result == null || result.Tables.Count == 0) return data;

            for (var i = 0; i < result.Tables.Count; i++)
            {
                using var table = result.Tables[i];
                ...
                for (var j = 0; j < rowCount; j++)
                {
                    ...
                    {
                        data.Add(drc[j][k]?.ToString()); // 使用?.操作符避免空引用异常
                    }
                }
            }
            return data;
        ...
        public static Dictionary<string, string[,]> DataSet2DictionaryStringArrayTwo(DataSet result)
        {
            ...
            {
                if (table is { Rows: not null, Columns: not null })
                {
        ...
        public static Dictionary<string, List<List<string>>> DataSet2DictionaryListList(DataSet result)
        {
            ...
            if (result is not { Tables: not null }) return dictionary;
            foreach (DataTable table in result.Tables)
            {
                if (table is { Rows: not null, Columns: not null })
                {
        ...
        public static SfExcelData DataSet2ExcelData(DataSet result)
        {
            var data = new SfExcelData();
            if (result == null || result.Tables.Count == 0) return data;
            ...
            for (var i = 0; i < result.Tables.Count; i++)
            {
                ...
                {
                    data.Add(new SheetData(tableName));
                    continue;
                }

                var sheetDataList = new string[rowCount, columnCount];
                for (var j = 0; j < rowCount; j++)
                {
                    for (var k = 0; k < columnCount; k++)
                    {
                        sheetDataList[j, k] = (drc[j][k]?.ToString()); // 使用?.操作符避免空引用异常
                    }
                }
                var sheetData = new SheetData(sheetDataList,tableName);
                data.Add(sheetData);
            }
            return data;
        }
