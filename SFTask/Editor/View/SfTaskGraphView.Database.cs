                ...
                try
                {
                    switch (p.propertyType)
                    {
                        ...
                        case SerializedPropertyType.Generic:
                            ...
                            {
                                if (string.IsNullOrEmpty(f.fieldValue)) break;
                                var data = JsonUtility.FromJson<UnityEventData>(f.fieldValue);
                                var callsProp = p.FindPropertyRelative("m_PersistentCalls.m_Calls");
                                if (callsProp == null) break;
                                callsProp.arraySize = data != null && data.calls != null ? data.calls.Count : 0;
                                for (int i = 0; i < callsProp.arraySize; i++)
                                {
                                    var callData = data.calls[i];
                                    ...
                                    if (!string.IsNullOrEmpty(callData.targetRef))
                                    {
                                        var typeName = callData.targetType;
                                        Type t = null;
                                        if (!string.IsNullOrEmpty(typeName)) t = ResolveType(typeName);
                                        if (callData.targetRef.StartsWith("scene-id://", StringComparison.OrdinalIgnoreCase))
                                        {
                                            var id = callData.targetRef.Substring("scene-id://".Length);
                                            var all = UnityEngine.Object.FindObjectsOfType<SFramework.SFTask.Module.SfUniqueId>(true);
                                            var hit = all.FirstOrDefault(x => x.Id == id);
                                            if (hit != null)
                                            {
                                                var go = hit.gameObject;
                                                targetObj = t != null && t != typeof(UnityEngine.GameObject) ? (UnityEngine.Object)go.GetComponent(t) : go;
                                            }
                                        }
                                        else if (callData.targetRef.StartsWith("scene://", StringComparison.OrdinalIgnoreCase))
                                        {
                                            var rel = callData.targetRef.Substring("scene://".Length);
                                            var go = UnityEngine.GameObject.Find(rel);
                                        ...
                                        {
                                            var path = AssetDatabase.GUIDToAssetPath(callData.targetRef);
                                            if (string.IsNullOrEmpty(path))
                                            {
                                                var res = UnityEngine.Resources.Load(callData.targetRef);
                                                if (res != null) targetObj = res;
                                            }
                                            else
                                            {
                                                targetObj = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path);
                                            }
                                        }
                                    }
                                    if (targetProp != null) targetProp.objectReferenceValue = targetObj;
                                    if (methodProp != null) methodProp.stringValue = callData.methodName ?? string.Empty;
                                    if (modeProp != null) modeProp.intValue = callData.mode;
                                    if (callStateProp != null) callStateProp.intValue = callData.callState;
                                    if (argsProp != null && callData.arguments != null)
                                    {
                                        ...
                                        UnityEngine.Object objArg = null;
                                        if (!string.IsNullOrEmpty(callData.arguments.objectRef))
                                        {
                                            Type argType = null;
                                            if (!string.IsNullOrEmpty(callData.arguments.assemblyTypeName)) argType = ResolveType(callData.arguments.assemblyTypeName);
                                            if (callData.arguments.objectRef.StartsWith("scene-id://", StringComparison.OrdinalIgnoreCase))
                                            {
                                                var id = callData.arguments.objectRef.Substring("scene-id://".Length);
                                                var all = UnityEngine.Object.FindObjectsOfType<SFramework.SFTask.Module.SfUniqueId>(true);
                                                var hit = all.FirstOrDefault(x => x.Id == id);
                                                if (hit != null)
                                                {
                                                    var go = hit.gameObject;
                                                    objArg = argType != null && argType != typeof(UnityEngine.GameObject) ? (UnityEngine.Object)go.GetComponent(argType) : go;
                                                }
                                            }
                                            else if (callData.arguments.objectRef.StartsWith("scene://", StringComparison.OrdinalIgnoreCase))
                                            {
                                                var rel = callData.arguments.objectRef.Substring("scene://".Length);
                                                var go = UnityEngine.GameObject.Find(rel);
                                                if (go == null) go = FindGameObjectByPathIncludingInactive(rel);
                                                if (go != null)
                                                {
                                            ...
                                            {
                                                var path = AssetDatabase.GUIDToAssetPath(callData.arguments.objectRef);
                                                if (string.IsNullOrEmpty(path))
                                                {
                                                    var res = UnityEngine.Resources.Load(callData.arguments.objectRef);
                                                    if (res != null) objArg = res;
                                                }
                                                else
                                                {
                                                    objArg = AssetDatabase.LoadAssetAtPath<UnityEngine.Object>(path);
                                                }
                                            }
                                        }

                                        if (objArgProp != null) objArgProp.objectReferenceValue = objArg;
                                        if (objAsmProp != null) objAsmProp.stringValue = callData.arguments.assemblyTypeName ?? string.Empty;
                                        if (intArgProp != null) intArgProp.intValue = callData.arguments.intValue;
                                        if (floatArgProp != null) floatArgProp.floatValue = callData.arguments.floatValue;
                                        if (stringArgProp != null) stringArgProp.stringValue = callData.arguments.stringValue ?? string.Empty;
                                        if (boolArgProp != null) boolArgProp.boolValue = callData.arguments.boolValue;
                                    }
                                }
                            }
                            break;
