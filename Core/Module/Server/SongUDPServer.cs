...
namespace Song.Core.Module.Server
{
    ...
    /// UDP服务 (已修正和优化版本)
    /// </summary>
    public class SongUDPServer:ISongUDPServer
    {
        #region 变量
        /// <summary>
        /// IP地址
        ...
        public static SongUDPServer Connect(int port, Action<string, int, string> onReceived = null)
        {
        ...
        /// <param name="onReceived">接收到消息时的回调函数</param>
        /// <returns>创建的服务实例</returns>
        public static SongUDPServer Connect(string ip, int port, Action<string, int, string> onReceived = null)
        {
            var server = new SongUDPServer();
            server.IP = ip;
            server.Port = port;
            ...
            try
            {
                ...
                // 创建并启动后台线程用于接收消息
                server._receiveThread = new Thread(server.ReceiveLoop);
                server._receiveThread.IsBackground = true; // 设置为后台线程，主程序退出时它会自动结束
                server._receiveThread.Start();
        ...
        public void Disconnect()
        {
            ...
            // 等待后台线程自然结束，设置一个超时以防万一
            if (_receiveThread != null && _receiveThread.IsAlive)
            {
                // Join会等待线程执行完毕。不再需要使用危险的Abort()。
                if (!_receiveThread.Join(1000))
                {
                    Debug.LogWarning("UDP接收线程在1秒内未能正常停止。");
                }
            }
            _receiveThread = null;
